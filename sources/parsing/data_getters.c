...
/*   Updated: 2024/03/26 09:55:56 by hlaadiou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_ambient	*get_ambient_data(t_pars *conf)
{
	...
	while (conf)
	{
		if (conf->flag == AMB)
		{
			if (!valid_float(conf->elements[0]))
				return (NULL);
			rgb = parse_color(conf->elements[1]);
			if (!rgb)
				return (NULL);
			amb = (t_ambient*)malloc(sizeof(t_ambient));
			if (!amb)
				return (NULL);
			amb->scale = ft_atof(conf->elements[0]);
			amb->color = rgb;
			break ;
		}
		conf = conf->next;
...
t_camera	*check_camera_data(t_pars *conf)
{
	...
	cam = NULL;
	pt = parse_coordinates(conf->elements[0]);
	if (!pt)
		return (NULL);
	vec = parse_coordinates(conf->elements[1]);
	if (!vec)
		return (free(pt), NULL);
	if (!normalized_range(vec))
		return (free(pt), free(vec), NULL);
	if (!valid_float(conf->elements[2]) || \
	ft_atof(conf->elements[2]) < 0.0 || ft_atof(conf->elements[2]) > 180.0)
		return (free(pt), free(vec), NULL);
	pt->w = 1.0;
	*cam = (t_camera){pt, vec, ft_atof(conf->elements[2])};
	return (cam);
}
...
t_light_src	*check_light_data(t_pars *conf)
{
	...
	if (!pt)
		return (NULL);
	if (!valid_float(conf->elements[1]) || ft_atof(conf->elements[1]) < 0.0 \
	...
	if (!rgb)
		return (free(pt), NULL);
	pt->w = 1.0;
	*light = (t_light_src){pt, ft_atof(conf->elements[1]), rgb};
	return (light);
}
...
t_object	*check_sp_data(t_pars *conf)
{
	...
	if (!rgb)
		return (free(pt), NULL);
	pt->w = 1.0;
	sp = _sphere(*pt, ft_atof(conf->elements[1]), rgb);
	if (!sp)
...
t_object	*check_cy_data(t_pars *conf)
{
	...
	if (!vec)
		return (free(pt), NULL);
	if (!valid_float(conf->elements[2]) || !valid_float(conf->elements[3]))
		return (free(pt), free(vec), NULL);
	rgb = parse_color(conf->elements[4]);
	if (!rgb)
		return (free(pt), free(vec), NULL);
	cy = _cylinder(*pt, *vec, ft_atof(conf->elements[2]), \
	ft_atof(conf->elements[3]), rgb);
	if (!cy)
		return (free(pt), free(vec), free(rgb), NULL);
	return (cy);
}
